...
where I2C: i2c::I2c,
{
  ...
  pub fn read_device_id(&mut self) -> Result<[u8;2], OPT4048Error<I2C::Error>> {
    let mut id = [0, 0];
    self.i2c.write_read(I2C_ADDRESS,&[OPT4048_REGISTER_DEVICE_ID], &mut id).map_err(OPT4048Error::I2C)?; 
    Ok(id)
  }

  pub fn read_lux(&mut self) -> Result<f32, OPT4048Error<I2C::Error>>  {
    let mut register_offset_2h = [0,0];
    let mut register_offset_3h = [0];
    self.i2c.write_read(I2C_ADDRESS,&[OPT4048_REGISTER_CHANNEL1], &mut register_offset_2h).map_err(OPT4048Error::I2C)?;   
    self.i2c.write_read(I2C_ADDRESS,&[OPT4048_REGISTER_CHANNEL1_EXTRA], &mut register_offset_3h).map_err(OPT4048Error::I2C)?;   
    
    let raw_2h = u16::from_be_bytes(register_offset_2h);
    let result_msb_ch1 = raw_2h & 0x0FFF;
    let exponent_ch1 = raw_2h & 0xF000 >> 12;
    let result_lsb_ch1 = register_offset_3h[0];
    let mantissa_ch1=(result_msb_ch1<<8) + result_lsb_ch1 as u16;
    let adc_codes_ch1 = mantissa_ch1<<exponent_ch1;
    Ok(LUX_MATRIX * adc_codes_ch1 as f32)
  }
